...
const addLogoSection = document.getElementById('add-logo-section');
...
function updateUI() {
  ...
  addLogoSection.style.display = currentUser ? 'block' : 'none';
...
// --- Логотипи ---
function getLogos(){ return JSON.parse(localStorage.getItem('logos')) || []; }
function saveLogos(logos){ localStorage.setItem('logos', JSON.stringify(logos)); }
...
function loadLogos(){
  ...
  document.querySelectorAll('.buy-button').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      ...
      if(balance < logo.price) return showToast('Недостатньо коштів', 'error');
